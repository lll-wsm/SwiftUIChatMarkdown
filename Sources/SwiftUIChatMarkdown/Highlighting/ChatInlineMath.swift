...
#if os(macOS)
import AppKit
public typealias ChatInlineMathPlatformImage = NSImage
#else
import UIKit
public typealias ChatInlineMathPlatformImage = UIImage
#endif

public struct ChatInlineMathSpan: Equatable, Sendable {
    public let latex: String
    public let source: String

    public init(latex: String, source: String) {
        self.latex = latex
        self.source = source
    }
}

public enum ChatInlineMathScanner {
    public enum Piece: Equatable, Sendable {
        case markdown(String)
        case math(latex: String, source: String)
        case literal(String)

        public var mathLatex: String? {
    ...
    public static let maxSpanCount = 16
    public static let maxSourceBytes = 200

    public static func pieces(in markdown: String) -> [Piece] {
        guard markdown.contains(#"\("#) || markdown.contains("$") else { return [.markdown(markdown)] }
        ...
        while cursor < markdown.endIndex {
            ...
            if markdown[cursor] == "$",
               !self.isEscaped(at: cursor, in: markdown)
            {
                let nextIdx = markdown.index(after: cursor)
                if nextIdx < markdown.endIndex {
                    let nextChar = markdown[nextIdx]
                    if nextChar != " ", nextChar != "\n", nextChar != "\r" {
                        if textStart < cursor {
                            pieces.append(.markdown(String(markdown[textStart..<cursor])))
                        }
                        let opener = cursor
                        let contentStart = nextIdx
        ...
        codeSpans: [Range<String.Index>]) -> Candidate?
    {
        ...
        while cursor < markdown.endIndex {
            ...
            if markdown[cursor...].hasPrefix(delimiter),
               !self.isEscaped(at: cursor, in: markdown)
            {
                if delimiter == "$" {
                    let prevIdx = markdown.index(before: cursor)
                    if markdown[prevIdx] == " " {
                        cursor = markdown.index(after: cursor)
                        continue
                    }
                ...
                return Candidate(
                    closeStart: cursor,
                    end: markdown.index(cursor, offsetBy: delimiter.count),
...
@MainActor
public enum ChatMathParseCache {
    private enum Result {
    ...
    public static let maxCommandCount = 128
    public static let maxSourceBytes = 5000
    private static let unsafeCommands = [#"\color"#, #"\colorbox"#, #"\textcolor"#]

    public static func mathList(latex: String) -> MTMathList? {
        guard !latex.isEmpty else { return nil }
        guard latex.utf8.count <= self.maxSourceBytes else { return nil }
...
@MainActor
public enum ChatInlineMathImageCache {
    public struct RenderedImage: @unchecked Sendable {
        public let image: ChatInlineMathPlatformImage
        public let baselineOffset: CGFloat

        public init(image: ChatInlineMathPlatformImage, baselineOffset: CGFloat) {
            self.image = image
    ...
    {
        self.image(latex: latex, fontSize: fontSize, textColor: theme.textColor, colorScheme: colorScheme)
    }
    ...
    public static func isSafeImageSize(_ size: CGSize, scale: CGFloat) -> Bool {
